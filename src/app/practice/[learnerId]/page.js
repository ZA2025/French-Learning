...
export default function PracticePage() {
  ...
  // Load progress from KV on mount
  useEffect(() => {
    if (!learnerId) return;
    ...
    const loadProgress = async () => {
      ...
      try {
        ...
        if (progress.modules && progress.modules.length > 0) {
          ...
          setActiveModuleId(progress.activeModuleId || progress.modules[0].id);
          
          const firstModule = progress.modules.find(m => m.id === (progress.activeModuleId || progress.modules[0].id));
          if (firstModule && firstModule.lessons.length > 0) {
            setActiveLessonId(progress.activeLessonId || firstModule.lessons[0].id);
          }
        }
      } catch (error) {
  ...
  // Auto-save progress to KV (debounced)
  const saveProgress = async (progressData) => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      try {
        ...
        // Silent save - no toast to avoid annoying users
      } catch (error) {
  ...
  // Save whenever modules or active IDs change
  useEffect(() => {
    if (!learnerId || isLoading) return;
  ...
  }, [modules, activeModuleId, activeLessonId, learnerId, isLoading]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-slate-600">Loading your practice...</p>
        </div>
      </div>
    );
  }
